...
int main() {
    ...
    for (int i = 2; i <= 2; i += 1) {
        ...
        hparticles[0].C_p_G = C_p_G_vals[i];
        ...
        snprintf(folder, sizeof(folder), "%s%s", drop_species[i], "/");
        //printf("%s\n", folder);
        sprintf(dir, "%s%s", PROJECT_DIR "verification/coupled_heat_mass_transfer/processed_data/", folder);
        printf("%s\n", dir);
        ...
        runSim(hparticles, NUMPART, iterate_particle, hparticles[0].diameter, periodic, domain_length,
               ...
               analytic, fixed_Re, model, fixed_tau, 1);
    }
