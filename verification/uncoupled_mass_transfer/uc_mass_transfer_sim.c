...
// Other properties
cl_float P_atm = 101325;  // Atmospheric pressure (Pa)
cl_float R_bar = 8314.5;
cl_float R = 287;  // Universal gas constant ()
...
int main() {
    ...
    for (float i = 1; i <= 10; i += 1) {
        hparticles[0].diameter = powf(1.1,power_half)/1000; //particle_diameter;
        ...
        timestep = tau * (i/10);
        log_step = timestep;
        sim_length = tau;//260 * tau;


        sprintf(prefix, "uc_mass_transfer");
        char c[50];
        float a = i/10.0;
        sprintf(c, "%.1f", a);
        char *token = strtok(c, ".");
        ...
        snprintf(folder, sizeof(folder), "%s%s%s%s%s", "uc_mass_transfer_", num[0], "_", num[1], "_tau/");
        sprintf(dir, "%s%s", PROJECT_DIR "/verification/uncoupled_mass_transfer/data/", folder);
...
//    log_step = timestep;
//
//    snprintf(folder, sizeof(folder), "%s%s%s%s%s", "uc_mass_transfer_", "0", "_", "001", "_tau/");
//    sprintf(dir, "%s%s", PROJECT_DIR "/verification/uncoupled_mass_transfer/data/", folder);
...
//    analytic = 0;
//
//    sprintf(prefix, "uc_an_mass_transfer");
//    snprintf(folder, sizeof(folder), "%s%s%s%s%s", "an_uc_mass_transfer_", "0", "_", "1", "_tau/");
//    sprintf(dir, "%s%s", PROJECT_DIR "/verification/uncoupled_mass_transfer/data/", folder);
//    hparticles[0].diameter = powf(1.1,power_half)/1000; //particle_diameter;
//    hparticles[0].effect_diameter = 0;
//    hparticles[0].fluid_viscosity = fluid_viscosity;
